...
End-to-End DevOps CI/CD Project – Sanic API → Docker → GitHub → DockerHub → AWS EC2 → GitHub Actions Auto-Deploy
This project demonstrates a complete DevOps delivery pipeline starting from local code development to automated deployment on AWS EC2 using Docker & GitHub Actions.

📌 Final Expected Output
    • Sanic API available over the internet
    • Running inside a Docker container on AWS EC2
...
Local Code → GitHub Repo → GitHub Actions CI/CD →
Build Docker Image → Push to DockerHub →
SSH into EC2 → Pull Latest Image → Run Container
...
Port 22, 8000 (or 80 if using NGINX)
Local Docker Installed
For testing build

🏁 PHASE 1 – Create & Dockerize Sanic App
📂 Project Structure
sanic-api/
 ├── app.py
 ├── requirements.txt
...
🏁 PHASE 3 – Setup DockerHub Repo & Credentials
    1. Create repository: girishsatya/sanic-api
    2. Generate DockerHub access token:
        ◦ DockerHub → Account → Security → Access Tokens → Read/Write

🏁 PHASE 4 – Launch & Configure EC2
On AWS EC2 (Amazon Linux 2023)
sudo dnf update -y
sudo dnf install docker -y
sudo systemctl start docker
sudo usermod -aG docker ec2-user
Re-login to apply group rule.

🏁 PHASE 5 – Deployment Script on EC2
SSH into instance:
ssh -i <your-key>.pem ec2-user@<EC2_PUBLIC_IP>
Create deployment folder and script:
mkdir -p ~/deploy
nano ~/deploy/deploy_sanic.sh
Paste:
#!/bin/bash
set -e
...
🏁 PHASE 7 – Create GitHub Actions CI/CD
Path:
...
git push
Go to: GitHub → Actions → Workflow Running
When green, open:
http://<EC2_PUBLIC_IP>:8000/
http://<EC2_PUBLIC_IP>:8000/health
...
"I built and deployed a Sanic-based API using Docker, DockerHub, GitHub Actions, and AWS EC2. I configured a fully automated CI/CD pipeline that builds and pushes Docker images on every commit to the main branch, then deploys the latest version on EC2 via SSH with zero manual intervention."
